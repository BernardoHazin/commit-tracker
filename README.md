...
- Create a copy of ``commitTracker/settings/local.py.example``:  
  `cp commitTracker/settings/local.py.example commitTracker/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/commitTracker/settings/base.py``.
- Adjust ``backend/commitTracker/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
